...
import { Camera, PenLine, Scan } from 'lucide-react'
...
type Step = 'choose' | 'photo' | 'barcode' | 'form'
...
export function AddProductModal({ open, onClose, onAdd }: AddProductModalProps) {
  ...
  const handleResult = (result: ClaudeProductResult) => {
    setPrefilled(result)
    setStep('form')
  }
  ...
  const stepTitle = {
    choose: 'Aggiungi prodotto',
    photo: 'Scatta le foto',
    barcode: 'Scansiona barcode',
  ...
  return (
    <Modal
      ...
      onClose={handleClose}
      title={stepTitle}
      fullscreen={step === 'photo' || step === 'barcode'}
    >
      <div onClick={step === 'choose' ? handleOpen : undefined}>
        {step === 'choose' && (
          <div className="space-y-3">
            <button
              ...
              className="w-full flex items-center gap-4 p-4 rounded-2xl border-2 border-warm-900 bg-warm-900 active:scale-[0.98] transition-all"
            >
              <div className="w-12 h-12 rounded-xl bg-white/20 flex items-center justify-center">
                <Scan size={22} className="text-white" />
              </div>
              <div className="text-left">
                <p className="font-medium text-white">Scansiona barcode</p>
                <p className="text-xs text-white/70">Più veloce — cerca nel database cosmetici</p>
              </div>
            </button>
            ...
            >
              <div className="w-12 h-12 rounded-xl bg-cream-100 flex items-center justify-center">
                <Camera size={22} className="text-warm-700" />
              </div>
              <div className="text-left">
                <p className="font-medium text-warm-900">Scatta foto</p>
                <p className="text-xs text-warm-500">AI legge etichetta e ingredienti INCI</p>
              </div>
            </button>
        ...
        {photoError && (step === 'photo' || step === 'barcode') && (
